...
$(document).ready(function() {
  ...
  // set up grid style sheets
  window.engine = new GSS(document);

  // draw rectangle for the example unconfirmed block in the intro text
  d3.select("#unconfirmed-key")
  .append("svg").attr("height", rectangleHeight).attr("width", 500)
  .append("rect")
  .attr("class", "unconfirmed")
  .attr("fill-opacity", 0)
  .attr("x", 0).attr("y", 0)
  .attr("width", pixelsPerBtc*10).attr("height", rectangleHeight);
  ...
  // draw rectangle for the example confirmed block
  ...
  // create vent handler for mouse clicks that shows and hides the intro
  d3.select("body")
  .on("click", function() {
   if ($("#intro-text").css("opacity") > 0) {
     ...
     $("#intro-container").animate({ opacity: 0.5}, 1000);
   }
  });
